...
#else
		geim_chg_connected(USB_CHG_TYPE__SDP);
#endif
	} else {
		ops->detach_handler();
...
#else
		geim_chg_connected(USB_CHG_TYPE__INVALID);
#endif
	}
...
#else
		geim_chg_connected(USB_CHG_TYPE__WALLCHARGER);
#endif
	} else {
		ops->detach_handler();
		fsa_cable_type = CABLE_TYPE_UNKNOWN;
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	SND(AUDIENCE_HANDSET, 48),
	SND(AUDIENCE_HANDSET_LOOP, 49),
	SND(CURRENT, 0x7FFFFFFE),
};
#undef SND
#endif
