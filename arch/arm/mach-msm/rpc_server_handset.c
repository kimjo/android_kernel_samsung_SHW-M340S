...
#if defined(CONFIG_MACH_VASTO) //kth_111114
#define HS_CHECK_PRE_EAR_JACK_K 0xF8
#endif
...
#if defined(CONFIG_MACH_VASTO) //kth_111114
	KEY(HS_CHECK_PRE_EAR_JACK_K, KEY_INFORM_EARJACK_PRE_CONNECTED),
#endif
	0
};
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	if( init == false ){
		wake_lock_init(&wake_lock, WAKE_LOCK_SUSPEND, "report_hs_key");
		init = true;
	}
...
//	printk("[KEY] key_parm: %d, key_code: %d, key: %d\n", key_parm, key_code, key);
	...
	if ((key != KEY_POWER) && !get_msm7x27a_det_jack_state()) {
		...
		if( (key_code == HS_REL_K) && (key == KEY_MEDIA) ) { //gopeace_ktoh M340L EL13 stop vibrate when earphone rejected (15 Earphone Key Test)
			input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
			pr_info("%s KEY_MEDIA /HS_REL_K test/%d\n", __func__, (key_code != HS_REL_K));
		}
#if defined(CONFIG_MACH_VASTO) //kth_111114
		if(key == KEY_INFORM_EARJACK_PRE_CONNECTED)
		{
//		    input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
		    pr_info("%s KEY_INFORM_EARJACK_PRE_CONNECTED %d=================================\n", __func__, (key_code != HS_REL_K));
		}
#endif
		return;	
	}
...
#endif
#if defined(CONFIG_MACH_VASTO) //kth_111114
	case KEY_INFORM_EARJACK_PRE_CONNECTED:
//		input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
		pr_info("%s KEY_INFORM_EARJACK_PRE_CONNECTED %d=================================\n", __func__, (key_code != HS_REL_K));
		break;
#endif
...
static int __devinit hs_probe(struct platform_device *pdev)
{
    ...
    #if defined(CONFIG_MACH_VASTO) //kth_111114
       input_set_capability(ipdev, EV_KEY, KEY_INFORM_EARJACK_PRE_CONNECTED);
    #endif
