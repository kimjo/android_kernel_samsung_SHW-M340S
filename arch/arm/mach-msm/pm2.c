...
#if defined(CONFIG_MACH_VASTO)
	if( acpuclk_max_rate > 0 ) saved_acpuclk_rate = acpuclk_max_rate;
#endif
...
static int msm_pm_prepare(void)
{
	gpiolib_power_show();
	vreg_show_refcnts(MSM_VREG_CMD_SLEEP);
	return 0;
}

static void msm_pm_finish(void)
{
	vreg_show_refcnts(MSM_VREG_CMD_WAKEUP);
	gpiolib_power_show();
}
...
static void msm_pm_restart(char str, const char *cmd)
{
	...
	msm_rpcrouter_close();
#if defined(CONFIG_MACH_VASTO)
	if( smem_flag ) smem_flag->info = 0x0;
#endif
