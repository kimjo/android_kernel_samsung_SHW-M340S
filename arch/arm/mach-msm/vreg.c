...
static struct msm7x2xa_vreg vasto_rev00_vreg[] = {
	[0] = {
		.circuit_name = "vreg_msmc1",
		.vreg_name = "msmc1",
		.initstate = 1,
...
void vreg_show_refcnts(unsigned int cmd)
{
...
void msm7x27a_vreg_init(unsigned int hwrev)
{
	...
	switch (hwrev) {
	default:
		_vreg = vasto_rev00_vreg;
		_vreg_size = ARRAY_SIZE(vasto_rev00_vreg);
		break;
	}
#else
	switch (hwrev) {
