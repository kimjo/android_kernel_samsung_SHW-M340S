...
#define READMD			0
#undef CONFIG_SENSORS_MMC328xMA_MAG 
...
int mmc328x_read( void ){
	...
	if (mmc328x_i2c_rx_data(data, 1) < 0) {
		printk(KERN_ERR "[COMPASS] %s[%d] : mmc328x_i2c_rx_data error!\n", __func__,__LINE__);
		return -EFAULT;
	}
	while (!(data[0] & 0x01)) {
		msleep(1);
		...
		MD_times++;
		if (MD_times > 2) {
			printk(KERN_ERR "[COMPASS] %s[%d] : mmc328x_i2c_rx_data error! data[0]=%d\n", __func__,__LINE__,data[0]);
			return -EFAULT;
		}
	}
...
static int mmc328x_ioctl( struct file *file, 
...
static const struct file_operations mmc328x_fops = {
	.owner		= THIS_MODULE,
	.open		= mmc328x_open,
...
static const struct miscdevice mmc328x_device = {
