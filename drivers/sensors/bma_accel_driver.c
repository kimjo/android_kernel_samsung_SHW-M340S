					...
					char *buf)
{
	...
	pr_err("%s: bma222_open_calibration() failed\n", __func__);

	return sprintf(buf, "%d,%d,%d\n",
		0, 0, 0);//these are dummy.
}
#if 0//to_be_fixd, not implemented yet
					...
					const char *buf, size_t count)
{
	...
	pr_err("%s: bma222_do_calibrate() failed, \n", __func__);

	return count;
}
#endif
static ssize_t bma222_acc_raw_data_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	int err;
	...
	if (err < 0) {
		pr_err("%s: bma222_read_accel_xyz() failed\n", __func__);
		return err;
	}
...
static DEVICE_ATTR(calibration, 0664,
		   bma222_calibration_show, NULL );//bma222_calibration_store);
static DEVICE_ATTR(raw_data, 0664,
		   bma222_acc_raw_data_show, NULL );
...
static DEVICE_ATTR(calibrate, S_IRUGO | S_IWUSR, bma_fs_cal, NULL);
...
//////////////////////////////////////////////////////////////////////////////
	...
	// [HSS] Support both address ( 0X18(old) and 0X08(new) ) (2010.09.29)
		...
		bma222_set_bandwidth(1); //bandwidth 250Hz => lowest to reduce noise
	...
	/* set sysfs for light sensor test mode*/
	err = sensors_register(accelerometer_sensor_device, data, accelerometer_sensor_attrs, "accelerometer_sensor");
	if(err) {
		printk(KERN_ERR "%s: cound not register accel sensor device(%d).\n", __func__, err);
	}
