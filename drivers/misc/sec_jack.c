...
#define FEATURE_HSSD //kth_temp 1
...
#define PMIC_EAR_ANT_GPIO 1 // PMIC GPIO 2
...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
    ...
    {
        pmic_gpio_set_value(PMIC_EAR_ANT_GPIO, 1);
    }
    else
    {
        pmic_gpio_set_value(PMIC_EAR_ANT_GPIO, 0);
    }
#endif
	...
	if (jack_type == SEC_HEADSET_4POLE) {
		...
		pr_info("sec_jack_set_type SEC_HEADSET_4POLE hi->send_key_irq_enabled = %d",hi->send_key_irq_enabled);
		...
		pr_info("sec_jack_set_type  hi->send_key_irq_enabled = %d",hi->send_key_irq_enabled);
		if (hi->send_key_irq_enabled) {
#if 0 //kth_temp 0
...
static void determine_jack_type(struct sec_jack_info *hi)
{
	...
	{
		pr_info("%s :get_det_jack_state = TRUE\n", __func__);
		msleep(50);
		sec_jack_set_type(hi, hi->pdata->get_adc_value());
	}
	else
       {   
		pr_info("%s :get_det_jack_state = FALSE\n", __func__);
		handle_jack_not_inserted(hi);
       }
 ...
 * or removal).
 */
#ifdef CONFIG_MACH_VASTO //kth_110926
int force_set_path;
#endif
...
#ifdef CONFIG_MACH_VASTO //kth_110926
static ssize_t select_force_device_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	pr_info("%s : operate nothing\n", __func__);

	return 0;
}       
static ssize_t select_force_device_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct sec_jack_info *hi = dev_get_drvdata(dev);
	struct sec_jack_platform_data *pdata = hi->pdata;
	int value = 0;
	...
	if (value == 1) {
		...
		force_set_path = false;
	}
	return size;
}
static DEVICE_ATTR(select_force_device, S_IRUGO | S_IWUSR | S_IWGRP , select_force_device_show, select_force_device_store);
#endif
...
static irqreturn_t sec_jack_send_key_irq_thread(int irq, void *dev_id)
{
	...
	int adc;
#endif
       pr_info("sec_jack_send_key_irq_thread\n");
...
#ifdef CONFIG_MACH_VASTO //kth_110926
	if (device_create_file(jack_selector_fs, &dev_attr_select_force_device) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_select_force_device.attr.name);
#endif
#endif
