...
#define CTRL_MASK		(SWITCH_OPEN | RAW_DATA | MANUAL_SWITCH | \
		WAIT | INT_MASK)
#define FSA880_REG_DEV_CTRL_MASK	(MANUAL_SWITCH | \
		INT_MASK)
...
#ifdef CONFIG_MACH_VASTO // add car-kit (h/w requeset)
...
#ifdef CONFIG_MACH_VASTO
static void fsa_init_detect(struct work_struct *work)
{
	struct fsausb_info *usbsw =
		container_of(work, struct fsausb_info, init_work.work); 
	int ret = 0;
		...
		const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&usbsw->init_work, fsa_init_detect);
