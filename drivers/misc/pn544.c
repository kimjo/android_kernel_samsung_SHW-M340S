			      ...
			      size_t count, loff_t * offset)
{
	...
	pr_info("%s : reading %zu bytes. irq=%s\n", __func__, count,
 ...
 fail:
	...
	pr_info("%s : wait_event_interruptible fail\n", __func__);
...
{
	struct pn544_dev *pn544_dev = filp->private_data;
	pr_info("%s pn544_dev_ioctl was called !!  command : %d\n", __func__,
		cmd);
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (ret) {
		...
		pr_info("%s : request_irq failed irq_num=%d\n", __func__,
			irq_num);
