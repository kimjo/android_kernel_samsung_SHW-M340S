...
#define S5K5CCAF_WRITE_LIST_BURST(A)	s5k5ccaf_sensor_write_list(A,(sizeof(A) / sizeof(A[0])),#A);
#else
#define S5K5CCAF_WRITE_LIST(A)			s5k5ccaf_sensor_burst_write_list(A,(sizeof(A) / sizeof(A[0])),#A);
#define S5K5CCAF_WRITE_LIST_BURST(A)	s5k5ccaf_sensor_burst_write_list(A,(sizeof(A) / sizeof(A[0])),#A);
#endif

#define S5K5CCAF_MODE_PREVIEW 0
#define S5K5CCAF_MODE_CAPTURE 1
...
static char mDTP = 0;
static char mInit = 0;
static char mMode = S5K5CCAF_MODE_PREVIEW;
...
static int s5k5ccaf_sensor_write_list(const u32 *list, int size, char *name)
{
	...
	for (i = 0; i < size; i++) {
		...
		if(subaddr == 0xffff)	{
			msleep(value);
			printk("[s5k5ccaf] msleep %d\n",value);
		} else {
		    if(s5k5ccaf_sensor_write(subaddr, value) < 0)   {
			    printk("[s5k5ccaf] sensor_write_list fail...-_-\n");
			    return -1;
		    }
		}
	}
...
int s5k5ccaf_fps_control(unsigned int fps)
{
	int rc = 0;
	
	printk("s5k5ccaf_fps_control : fps = %d\n", fps);

	if( mScene!=EXT_CFG_SCENE_OFF ){
		printk("s5k5ccaf_fps_control : scene mode is set, skip fps setting!!\n");
		return rc;
	}
	...
	case EXT_CFG_FRAME_FIX_24:
		S5K5CCAF_WRITE_LIST(s5k5ccaf_fps_24fix);
		break;
...
{
#if 0 // vasto doesn't use iso function.
	switch(value)
	{
...
void s5k5ccaf_scene_control(char value)
{
	...
	if( mInit==0 || mLastScene== EXT_CFG_SCENE_TEXT ) {
		...
		s5k5ccaf_af_control(EXT_CFG_AF_SET_MACRO);
	}
	mLastScene = value;
}
	...
	switch(value)
	{
		...
		case EXT_CFG_AF_SET_NORMAL :{
			if( mCurAfMode != EXT_CFG_AF_SET_NORMAL ){
				...
				printk("already EXT_CFG_AF_SET_NORMAL is set! skip!!");
			}
		}
		break;
		...
		case EXT_CFG_AF_SET_MACRO :{
			if( mCurAfMode != EXT_CFG_AF_SET_MACRO ){
				...
				printk("already EXT_CFG_AF_SET_MACRO is set! skip!!");
			}
		}
		break;
		...
		s5k5ccaf_sensor_read(0x0F12, &af_status);
		CAMDRV_DEBUG("<=PCAM=> AF 2nd check status : %x", af_status);
		return af_status;
		}
		break;
...
void s5k5ccaf_CPU_control(char value)
{
	switch(value)
	{
...
static int s5k5ccaf_sensor_ext_config(void __user *arg)
{
	...
	switch(ctrl_info.mode)
	{
		case EXT_CFG_GET_INFO:{
			...
			s5k5ccaf_sensor_write(0xFCFC, 0xD000);	// shutter speed				
			s5k5ccaf_sensor_write(0x002C, 0x7000);
			s5k5ccaf_sensor_write(0x002E, 0x2A14);
			s5k5ccaf_sensor_read(0x0F12, &lsb);
			s5k5ccaf_sensor_read(0x0F12, &msb);
			
			s5k5ccaf_sensor_write(0xFCFC, 0xD000);	// iso speed rate				
			s5k5ccaf_sensor_write(0x002C, 0x7000);
			s5k5ccaf_sensor_write(0x002E, 0x2A18);
			s5k5ccaf_sensor_read(0x0F12, &rough_iso);
		...
		case EXT_CFG_AF_CONTROL:
		{
			...
			{
				ctrl_info.value_3 = s5k5ccaf_af_control(ctrl_info.value_1);
			}
		}
		break;
		...
		{
			s5k5ccaf_CPU_control(ctrl_info.value_1);
		}
		break;
...
int s5k5ccaf_mipi_mode(int mode)
{
	...
	if (!config_csi2) {
		...
		s5k5ccaf_csi_params.settle_cnt = 0x14;
...
int wait_sensor_mode(int mode, int interval, int cnt)
{
	...
	printk("s5k5ccaf wait_sensor_mode E\n");
	do {
		s5k5ccaf_sensor_write(0x002C, 0x7000);
	...
	printk("s5k5ccaf wait_sensor_mode X\n");
...
//	unsigned short read_value;
	...
	if( mDTP ) {
		if( mInit==0 ) {
			S5K5CCAF_WRITE_LIST_BURST(s5k5ccaf_DTP_init0);
			wait_sensor_mode(S5K5CCAF_MODE_PREVIEW, 10, 10);
		}
	}
	else{
		if( mInit==0 ) {
...
void s5k5ccaf_set_capture(void)
{
	...
	CAMDRV_DEBUG("s5k5ccaf_set_capture");
	...
	wait_sensor_mode(S5K5CCAF_MODE_CAPTURE, 10, 40);	//remove duplicated delay : 20 -> 40
	mMode = S5K5CCAF_MODE_CAPTURE;
}
...
#endif		
		msleep(300); // wait for initing af position
...
static int s5k5ccaf_check_sensor_id(void)
{
	...
	printk("[S5K5CCGX] s5k5ccaf_check_sensor_id()\n");
...
static int s5k5ccaf_regs_table_write(char *name)
{
	...
	*(data_buf + 10) = '\0';
	...
	start = sec_strstr(s5k5ccaf_regs_table, name);
	...
	while (1) { 
		...
		if ((reg == NULL) || (reg > end))
		{
			printk("[s5k5ccaf] write end of %s\n",name);
			break;
		}
		...
		{
			printk("[s5k5ccaf] it's not hex number %s\n",data_buf);
			continue;
		}
		...
		else
		{
			...
			{
				printk("[s5k5ccaf] %s fail on sensor_write : addr[0x%04x], value[0x%04x]\n", __func__, addr, value);
			}
		}
	}
...
int s5k5ccaf_sensor_release(void)
{
	...
	{
		printk("[S5K5CCAF] wait change lens position\n");
		s5k5ccaf_af_control(EXT_CFG_AF_SET_NORMAL);
		msleep(150);
	}
	else
		printk("[S5K5CCAF] mAfMode : %d\n", mAfMode);
