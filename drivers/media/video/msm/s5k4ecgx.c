...
ctrl_info.value_1 = s5k4ecgx_status.flash_exifinfo; */
	...
	case EXT_CFG_SET_FLASH_MODE:	/*yjh_flash */
		if (ctrl_info.value_1 != EXT_CFG_FLASH_TURN_ON
			...
			pr_info("hoonB = %d", ctrl_info.value_1);
...
void cam_pw(int status)
{
	...
	/*pr_info("[S5K4ECGX] Camera Sensor Power : ");*/
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (device_create_file(s5k4ecgx_dev, &dev_attr_flash) < 0) {
				 ...
				 struct msm_sensor_ctrl *s)
{
	...
	gpio_tlmm_config(GPIO_CFG
			 (15, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
			  GPIO_CFG_16MA), GPIO_CFG_ENABLE);
	usleep(5000);
	gpio_set_value(CAM_5M_STBY, 1);
	 /*STBY*/ usleep(1000);
	gpio_set_value(CAM_5M_RST, 1);
	 /*RESET*/ rc = s5k4ecgx_sensor_init_probe();
	if (rc < 0) {
		cam_pw(0);	/*sleep current
				   issue for sensor disconnection */
		goto probe_done;
	}
	...
	udelay(100);
	gpio_set_value(CAM_5M_STBY, 0);
	 /*STBY*/ udelay(100);
	gpio_set_value(CAM_5M_RST, 0);
	udelay(100);
