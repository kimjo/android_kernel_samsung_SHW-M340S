...
static void stmpe_late_resume(struct early_suspend *h)
{
	struct stmpe_keypad *keypad =
			 container_of(h, struct stmpe_keypad, early_suspend);

	printk(KERN_INFO" stmpe KEY LED ON ................\n");
#if 0
	stmpe_KeyLED_On(keypad);
#endif
}

static void stmpe_early_suspend(struct early_suspend *h)
{
	struct stmpe_keypad *keypad =
			container_of(h, struct stmpe_keypad, early_suspend);

	printk(KERN_INFO" stmpe KEY LED OFF ..............\n");
#if 0
	stmpe_KeyLED_Off(keypad);
#endif
}
#endif
...
static DEVICE_ATTR(qwerty_keyled, S_IWUSR, NULL, slide_test);
...
static int __devinit stmpe_keypad_chip_init(struct stmpe_keypad *keypad)
{
	...
	/* GPIO direction */
	ret = stmpe_reg_write(stmpe, STMPE_GPIO_DIR_LOW, 0x80);
	if (ret < 0)
		return ret;
	...
	ret = stmpe_reg_write(stmpe, STMPE_GPIO_DIR_HIGH, 0x02);
	if (ret < 0)
		return ret;

	ret = stmpe_reg_write(stmpe, STMPE1801_REG_GPIO_RE_LSB, 0x01);
	if (ret < 0)
		return ret;

	/* Edge detection */
	ret = stmpe_reg_write(stmpe, STMPE_GPIO_FE_LOW, 0x01);
	if (ret < 0)
		return ret;
	...
	ret = stmpe_reg_write(stmpe, STMPE1801_REG_GPIO_CLR_LSB, 0x80);
	if (ret < 0)
		return ret;
	...
	ret = stmpe_reg_write(stmpe, STMPE1801_REG_GPIO_SET_HIGH, 0x02);
	if (ret < 0)
		return ret;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	keypad->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	keypad->early_suspend.suspend = stmpe_early_suspend;
	keypad->early_suspend.resume = stmpe_late_resume;

	register_early_suspend(&keypad->early_suspend);
#endif

	kp_flip_dev = device_create(sec_class, NULL, 0, NULL,
		 "sec_qwerty_keyled");
	if (!kp_flip_dev)
		printk(KERN_INFO"Failed to create device(sec_qwerty_keyled)!\n");

	if (device_create_file(kp_flip_dev, &dev_attr_qwerty_keyled) < 0)
		printk(KERN_INFO"Failed to create file(%s)!\n",
			dev_attr_qwerty_keyled.attr.name);
