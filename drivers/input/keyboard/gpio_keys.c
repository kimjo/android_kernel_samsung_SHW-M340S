...
extern int param_debug_level; 

static int lockup_capture_check(int keycode, u8 keypress) 
{ 
        static enum { NONE, STEP1, STEP2,STEP3,STEP4,STEP5,STEP6,STEP7,STEP8} state = NONE; 
        ...
        { 
                ...
                case STEP8: 
                        if (keycode == LOCKUP_THIRD_KEY&& keypress) { 
                                if ((param_debug_level == KERNEL_SEC_DEBUG_LEVEL_LOW)) { 
...
static void gpio_keys_report_event(struct gpio_button_data *bdata)
{
	...
	input_sync(input);
#if defined(CONFIG_MACH_TREBON) 
	lockup_capture_check(button->code, state); 
#endif 
