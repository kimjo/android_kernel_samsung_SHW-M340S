...
#include "zinitix_touch_bt4x3_firmware_geim_R009.h"
...
static ssize_t tkey_rawcounter_show3(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	uint8_t tkey_rawcounter[1] = {0};
	uint8_t i2c_addr = 0x00AB;
	int ret;

	ret = ts_read_data(g_touch_dev->client, i2c_addr,
			tkey_rawcounter, 2);
	if (ret <= 0) {
		...
		if (ret <= 0) {
			printk(KERN_ERR "[TSP] %s rawcounter read fail!!!!\n",
				__func__);
			goto error_return;
		}
	}
	...
	return snprintf(buf, sizeof(int), "%d", tkey_rawcounter[0]);

error_return:
	return snprintf(buf, sizeof(int), "-1");
}
