...
//#define DEBUG_IPC
#ifdef DEBUG_IPC
#define ipcprintk(s, args...) printk("[DPRAM] %s:%d - " s, __func__, __LINE__,  ##args)
#else
...
static int dpram_get_dgs(void)
{
	...
	typedef struct{
		UINT8 header_code[4];  // magic ³Ñ¹ö   // {0x7F,0xAA,0xAF,0x7E}
		UINT8 model[20];       // ¸ðµ¨¸í
		UINT8 nature[40];      // ÃâÇ×Áö
		UINT8 custt_code[8];   // °Å·¡¼±
		UINT8 date[14];        // ¹öÀü ¹ßÇàÀÏ
		UINT8 charger[24];     // ´ã´çÀÚ
		UINT8 version[20];     // SW ¹öÀü¸í
		UINT8 checksum[10];    // binary ckecksum
		UINT8 crcsum[10];      // binary CRC
		UINT8 Unique_Number[20];  // UN number
		UINT8 mem_name[20];    // ¸Þ¸ð¸® ¸í
		UINT8 sec_code[20];    // 
		UINT8 etc[40];         // ±âÅ¸
    }NAND_HEAD_INFO;
	NAND_HEAD_INFO header_info = {{0x7F,0xAA,0xAF,0x7E}, {"GT-S5700"}, {"EUR-Open"}, {"TIM"},
								{"2010-09-02"}, {"LKH"}, {"I5500AIJC3"}, {"C721"}, {"41D7"}, 
...
#else
	pr_err("\n ***** temporarily blocked!! MUST BE IMPLEMENTED!! (%s:%d) ****** \n\n", __func__, __LINE__);
#endif
}
...
static irqreturn_t dpram_interrupt(int irq, void *dev_id)
{
	...
	ipcprintk("################# Start interrupt handler ###################\n");	
#ifdef DEBUG_IPC
	print_smem();
#endif
...
//	wake_lock_timeout(&dpram_wake_lock, HZ/2);
	...
	ipcprintk("%s :################# Complete interrupt handler ################### \n", __func__);
...
#if 1 //trebon_dpram_port_temp test with smem_alloc2 before modem is setup
	do {
		extern void *smem_do_alloc(unsigned id, unsigned size_in);
		SmemBase = (volatile unsigned char *)(smem_do_alloc(SMEM_ID_VENDOR0, 0x4000*2));
	} while(0);
#else
...
void dump_ramdump_enable()
{
	if(smem_flag) smem_flag->info = 0xA9A9A9A9;
}
		                    ...
		                    unsigned long count, void *data)
{
	...
	if (buf[0] == '0') {			// low (no RAM dump)
		dump_enable_flag = 0;
		if(smem_flag) smem_flag->info = 0xAEAEAEAE;
