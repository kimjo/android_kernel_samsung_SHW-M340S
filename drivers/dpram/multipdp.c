...
//#define DEBUG_IPC
#ifdef DEBUG_IPC
#define ipcprintk(s, args...) printk("[MULTIPDP] %s:%d - " s, __func__, __LINE__, ##args)
#else
#define ipcprintk(s, args...)
#endif	/* DEBUG */
...
struct pdp_info {
	...
	union {
		...
		struct {
			struct tty_driver	tty_driver[7];	// CSD, ROUTER, GPS, XGPS, SMD, CPLOG, LOOPBACK
			int			refcount;
...
int	multipdp_rx_cback( char *buf, int len)
{
	...
	for( i=0; i < len; i++)
	{
		if (buf[i] == 0x7f) {
...
#ifdef DEBUG_IPC
			printk("==== 1 (Start byte) buf[%d] = 0x%x\n", i, buf[i]);
			printk("==== 1 (pdp_hdr[0]) buf[%d] = 0x%x\n", i+1, buf[i+1]);
			printk("==== 1 (pdp_hdr[1]) buf[%d] = 0x%x\n", i+2, buf[i+2]);
			printk("==== 1 (pdp_hdr[2]) buf[%d] = 0x%x\n", i+3, buf[i+3]);
			printk("==== 1 (pdp_hdr[3]) buf[%d] = 0x%x\n", i+4, buf[i+4]);
			printk("==== 1 (payload[0]) buf[%d] = 0x%x\n", i+5, buf[i+5]);

			printk("==== 1 (payload[size] buf[%d] = 0x%x\n", i+1+phdr->len-1, buf[i+1+phdr->len-1]);
			printk("==== 1 (End byte) buf[%d] = 0x%x\n", i+1+phdr->len, buf[i+1+phdr->len]);
			
			printk("==== 2 (Start byte)buf[%d] = 0x%x\n", i+1+phdr->len+1, buf[i+1+phdr->len+1]);
			printk("==== 2 (pdp_hdr[0]) buf[%d] = 0x%x\n", i+1+phdr->len+2, buf[i+1+phdr->len+2]);
#endif
			...
			//i = i + phdr->len + 2;
			i = i + phdr->len+1;			
//			dprintk("multipdp_demux : end ret = %d, next i = %d\n", ret, i);
...
static int vs_open(struct tty_struct *tty, struct file *filp)
{
	...
	tty->low_latency = 0; // change 1 to 0
	dev->vs_dev.tty = tty;
...
static int multipdp_vs_read(struct pdp_info *dev, char *buf, size_t len)
{
	...
	{	
		printk("###########################################\n");
		for(iCount=0; iCount<80; iCount=iCount+4)
			ipcprintk("file_header : [%x]\n", *((unsigned int *)(&buf[iCount])));
		printk("###########################################\n");
	}	
#endif
	...
	if(len > MAX_RX_BUFF_LEN) {
		...
		if(prx_buf == NULL)
		{
			printk("%s : [Error] prx_buf is null!\n", __func__);
			return 0;
		}
		...
		if(ret != len)
		{
			printk("%s : [Error] ret != len!\n", __func__);
			return ret;
		}


		if(dev->vs_dev.tty == NULL ||  (dev->id == 1 && !fp_vsCSD) || (dev->id == 5 && !fp_vsGPS) || (dev->id == 8 && !fp_vsEFS)|| (dev->id == 25 && !fp_vsSMD)
            	|| (dev->id == 6 && !fp_vsEXGPS)|| (dev->id == 29 && !fp_vsCPLOG) || (dev->id == 31 && !fp_vsLOOPBACK))		 
		{
		...
		if (ret > 0 && dev->vs_dev.tty != NULL) {
			ret = multipdp_tty_insert_data(dev->vs_dev.tty, prx_buf, ret); //execute
			if( ret > 0 )
			{
...
static int multipdp_demux(char *buf, int size)
{
	...
	ipcprintk("into channel id = %d, payload(with pdp_hdr) length = %d,  packet size = %d \n", hdr.id, hdr.len, size);
...
#else
static int multipdp_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
#endif
{
	int ret, adjust;
	pdp_arg_t pdp_arg;
